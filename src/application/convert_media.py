...
def convert_media(item: MediaItem, src_path: Path, dest_path: Path, app_root: Path, log) -> tuple[bool, Path | None]:
    ext = item.extension.lower()
    if ext in ('.heic', '.heif'):
        output = compat_output_path(dest_path, '.jpg')
        ok = convert_heic_to_jpg(app_root, src_path, output, log)
        if ok:
            copy_metadata(app_root, src_path, output, log)
        return ok, output if ok else None

    if item.is_video and ext in ('.mov', '.m4v'):
        output = compat_output_path(dest_path, '.mp4')
        ok = convert_video_to_mp4(app_root, src_path, output, log)
        if ok:
            copy_metadata(app_root, src_path, output, log)
        return ok, output if ok else None
